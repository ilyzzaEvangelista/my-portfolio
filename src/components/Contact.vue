...
<template>
  <section id="contact" class="py-28 px-6 section-alt">
    <div class="max-w-6xl mx-auto">
      <SectionHeading number="05" title="Let's Connect" />
      ...
      <div class="grid lg:grid-cols-2 gap-10">
        <div class="space-y-4">
          ...
          >
            <div class="w-11 h-11 rounded-xl bg-[var(--color-tag-bg)] flex items-center justify-center shrink-0 group-hover:opacity-80 transition-opacity">
            ...
            <div>
              <p class="text-xs text-muted uppercase tracking-wider">{{ info.label }}</p>
              <p class="text-sm font-medium text-emphasis">{{ info.value }}</p>
            </div>
          </a>
          ...
          <div class="flex gap-3 pt-4">
            <a
              ...
              class="w-11 h-11 rounded-xl glass-card flex items-center justify-center text-muted hover:text-primary hover:-translate-y-0.5 transition-all"
            >
        ...
        <form class="glass-card p-6 md:p-8 space-y-4" @submit.prevent="handleSubmit">
          <input v-model="form.name" type="text" placeholder="Your Name" required class="input-field" />
          <input v-model="form.email" type="email" placeholder="Your Email" required class="input-field" />
          <textarea
